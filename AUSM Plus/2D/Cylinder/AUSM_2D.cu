...
cell::cell(double *initial)
{
	...
	stateVar[1]=0.0;
	stateVar[2]=0.0;
	stateVar[3]=(gammma-1)*(initial[3]-0.5*(pow(stateVar[1],2)+pow(stateVar[2],2))/stateVar[0]);
}
...
__global__ void Boundary(cell *domain,double *initial)
{
	...
	if(domain[x].flag==2)
	{
		if(domain[x].face[y]<1 || domain[x].face[y]>25000)
		{
			note=y;
		}
...
void ausmplus(double *initial,double timesteps, double deltat)
{
	...
	set_nodes<<<25000,4>>>(d_node,d_domain,d_boundary,d_initial);
	set_neighbour<<<25000,4>>>(d_domain);
	calculate_norm<<<25000,4>>>(d_domain);
	read_values<<<25000,4>>>(d_domain);
	...
	for (double t = 0; t < timesteps*deltat; t+=deltat)
	{
		...
		cudaDeviceSynchronize();
		if((int)(t/deltat)%50000==0)
		{
			cudaMemcpyAsync(&domain[0],d_domain,25000*sizeof(cell),cudaMemcpyDeviceToHost,stream3);
			visual(domain,t);
		}	
	}
