...
__global__ void evaluate(cell *domain,double deltat)
{
	int x=blockIdx.x;
	int y=threadIdx.x;
	double vol=abs(0.5*(domain[x].nodes[0][0]-domain[x].nodes[2][0])*(domain[x].nodes[1][1]-domain[x].nodes[3][1])+\
...
__global__ void Boundary(cell *domain,double *initial)
{
	...
	else if(domain[x].flag==2)
	{
		if(threadIdx.x>0)
		{
			domain[x].stateVar[y]=2*domain[x].stateVar[y]-domain[(int)domain[x].face[3]].stateVar[y];
		}
		else
...
void ausmplus(double *initial,double timesteps, double deltat)
{
	...
	double *boundary=new double[600*2];
	...
	cudaMemcpyAsync(d_boundary,&boundary[0],600*2*sizeof(double),cudaMemcpyHostToDevice,stream3);
...
/*
	...
	for (int i = 0; i < 25000; ++i)
	{
		...
		cout<<"("<<domain[i].norms[0][0]<<","<<domain[i].norms[0][1]<<")"<<","<<"("<<domain[i].norms[1][0]<<","<<domain[i].norms[1][1]<<")";
		cout<<"("<<domain[i].norms[2][0]<<","<<domain[i].norms[2][1]<<")"<<","<<"("<<domain[i].norms[3][0]<<","<<domain[i].norms[3][1]<<")"<<endl;
		//cout<<domain[i].norms[0][1]<<","<<domain[i].norms[1][1]<<","<<domain[i].norms[2][1]<<","<<domain[i].norms[3][1]<<"		"<<domain[i].flag<<"	"<<i<<endl;
		//cout<<endl;
	}
*/
	delete[] domain;
